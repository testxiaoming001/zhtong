...
class YoujiaPay extends ApiPayment
{
    ...
    private function pay($order,$type='alipay'){
        ...
        $merkey = '7aMJkR94YYc25KOEzrmPxHwjTpuvXw09';
        $url = 'http://admin.hjbhzl.cn/gateway/index/unifiedorder?format=json';
...
//        return "http://caishen.sviptb.com/pay.php?".(http_build_query($data));
        ...
        if($result['code'] != '200' )
        {
            Log::error('Create YoujiaPay API Error:'.$result['msg']);
            throw new OrderException([
                'msg'   => 'Create YoujiaPay API Error:'.$result['msg'],
                'errCode'   => 200009
            ]);
        }
        return 'http://admin.hjbhzl.cn/'.$result['url'];
    }
    ...
    {
        $notifyData = $_POST;
        Log::notice("YoujiaPay notify data".json_encode($notifyData));
        ...
        echo "error";
        Log::error('YoujiaPay API Error:'.json_encode($notifyData));
    }
}