...
class JishifuPay extends ApiPayment
{
    /**
     * 统一下单
     */
    private function pay($order,$type='alipay'){
        ...
        $merkey = 'afrxxU6EcBLSSM9O58rjwyKuHEl3umGS';
        $url = 'http://api.jshipay.com/index/unifiedorder?format=json';
...
//        return "http://caishen.sviptb.com/pay.php?".htmlspecialchars(http_build_query($data));
        ...
        if($result['code'] != '200' )
        {
            Log::error('Create JishifuPay API Error:'.$result['msg']);
            throw new OrderException([
                'msg'   => 'Create JishifuPay API Error:'.$result['msg'],
                'errCode'   => 200009
            ]);
        }
        return $result['url'];
    ...
    public function query($notifyData){
        ...
        $url = 'http://api.jshipay.com/index/getorder';

        $key = 'afrxxU6EcBLSSM9O58rjwyKuHEl3umGS';
        $data=array(
            'appid'=>'1097505',
        ...
        Log::notice('query JishifuPay  API notice:'.json_encode($result));
        if(  $result['code'] != '200' ){
            Log::error('query JishifuPay  API Error:'.$result['msg']);
            return false;
        }
    ...
    {
        $notifyData = $_POST;
        Log::notice("JishifuPay notify data".json_encode($notifyData));
        ...
        echo "error";
        Log::error('JishifuPay API Error:'.json_encode($notifyData));
    }
}