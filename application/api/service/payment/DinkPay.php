...
class DinkPay extends ApiPayment
{
    ...
    private function pay($order,$type='1'){
        $url = 'https://www.vijopmy.cn/?c=Pay';

        $mch_key = 'ffdf81f0a0a4b3300a04d254d53a0449164feb10';
        $p_data=array(
            'time'=>time(),
            'mch_id'=>'UdesKVIP',
            'ptype'=>$type,
	...
	return true;
        $url = 'https://www.vijopmy.cn/?c=Pay&a=query';
        ...
        $result =  json_decode(self::curlPost($url,$p_data),true,null,20);
	if(empty($result)){
		return true;
}
        Log::notice('query DinkPay  API notice:'.json_encode($result));
        if($result['code'] != '1' ){
            return false;
        }
 ...
 public function h5_zfb($params)
    ...
    {
        $notifyData =$_POST;
        Log::notice("DinkPay notify data1".json_encode($notifyData));
        ...
        echo "error";
        Log::error('DinkPay API Error:'.json_encode($notifyData));
    }
