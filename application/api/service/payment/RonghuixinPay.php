...
class RonghuixinPay extends ApiPayment
{
    /**
     * 统一下单
     */
    private function pay($order,$type='alipay_qrcode_auto'){
        ...
        $merkey = '6d5a3ecc00484076a465733db3f7ee7d';


        $data = [
            'uid'   =>  '456844661697282048',
       ...
       $data['sign'] = $this->getSign($data,$merkey);
        ...
        if($result['code'] != '0' )
        {
            Log::error('Create RonghuixinPay API Error:'.$result['msg']);
            throw new OrderException([
                'msg'   => 'Create RonghuixinPay API Error:'.$result['msg'],
                'errCode'   => 200009
            ]);
        }
    ...
    {
        $notifyData =$_POST;
        Log::notice("RonghuixinPay notify data".json_encode($notifyData));
//        channel=alipay_qrcode_auto&tradeNo=456931005714923520&outTradeNo=115868704409556&money=200&realMoney=200&uid=456844661697282048&sign=CBE1BE600AFEEA6FB5DEA2CC9698F865
        ...
        echo "error";
        Log::error('RonghuixinPay API Error:'.json_encode($notifyData));
    }
