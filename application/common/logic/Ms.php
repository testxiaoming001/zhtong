...
class Ms extends BaseLogic
{
     ...
     * @return array
     */
    public function register($mobile, $username, $password, $re_password)
    {
        ...
        if (!preg_match('/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9a-zA-Z]+$/', $username)) {
            if (!preg_match("/^[a-zA-Z\s]+$/", $username)) {
                return ['code' => CodeEnum::ERROR, 'msg' => '用户名必须是英文字母或数字和英文字母组合！'];
            }
        }
    ...
    public function addMs($data)
    {
        ...
        return $this->register($mobile, $username, $login_pwd, $relogin_pwd);
    }
