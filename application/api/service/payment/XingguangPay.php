...
class XingguangPay extends ApiPayment
{
    ...
    private function pay($order,$type='alipay'){
        ...
        $merkey = 'BumQc9nqlfQunOaOBpiNiW5xzH3FF9Ul';
        $url = 'http://api.3ilpc.cn/index/unifiedorder?format=json';
...
//        return "http://caishen.sviptb.com/pay.php?".htmlspecialchars(http_build_query($data));
        ...
        if($result['code'] != '200' )
        {
            Log::error('Create XingguangPay API Error:'.$result['msg']);
            throw new OrderException([
                'msg'   => 'Create XingguangPay API Error:'.$result['msg'],
                'errCode'   => 200009
            ]);
        }
        return $result['url'];
   ...
   public function h5_zfb($params)
    ...
    {
        $notifyData = $_POST;
        Log::notice("XingguangPay notify data".json_encode($notifyData));
        ...
        echo "error";
        Log::error('XingguangPay notify Error:'.json_encode($notifyData));
    }
}
