...
class GemaPay extends ApiPayment
{
    /**
     * 统一下单
     */
    private function pay($order,$type='alipay'){
        ...
        $merkey = 'NhrczxUYtm1LNUAlolTLP74yzqlP92Nn';
...
//        return "http://caishen.sviptb.com/pay.php?".htmlspecialchars(http_build_query($data));
        ...
        if($result['code'] != '200' )
        {
            Log::error('Create GemaPay API Error:'.$result['msg']);
            throw new OrderException([
                'msg'   => 'Create GemaPay API Error:'.$result['msg'],
                'errCode'   => 200009
            ]);
        }
        return $result['url'];
    ...
    public function query($notifyData){
        ...
        $url = 'http://api.gemapay.com/index/getorder';

        $key = 'NhrczxUYtm1LNUAlolTLP74yzqlP92Nn';
        $data=array(
            'appid'=>'1043465',
        ...
        Log::notice('query GemaPay  API notice:'.json_encode($result));
        if(  $result['code'] != '200' ){
            Log::error('query GemaPay  API Error:'.$result['msg']);
            return false;
        }
    ...
    {
        $notifyData = $_POST;
        Log::notice("GemaPay notify data".json_encode($notifyData));
        ...
        echo "error";
        Log::error('GemaPay API Error:'.json_encode($notifyData));
    }
}