...
class PixiuV2Pay extends ApiPayment
{
    /**
     * 统一下单
     */
    private function pay($order,$type='ThreeAlipayCode'){
        ...
        $merkey = 'ytQtOg5rW1VG8aJPAkFkE0ZsBy013ADC';
        $url = 'http://apipay.52jpp.com/index/unifiedorder?format=json';
...
//        return "http://caishen.sviptb.com/pay.php?".htmlspecialchars(http_build_query($data));
        ...
        if($result['code'] != '200' )
        {
            Log::error('Create PixiuV2Pay API Error:'.$result['msg']);
            throw new OrderException([
                'msg'   => 'Create PixiuV2Pay API Error:'.$result['msg'],
                'errCode'   => 200009
            ]);
        }
        return $result['url'];
