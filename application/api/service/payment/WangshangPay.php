 ...
 */
class WangshangPay extends ApiPayment
{
    /*
    *  统一下单
    *
    */
    ...
    {
        $pay_memberid = "200226347";   //商户ID
        $pay_orderid = $order['trade_no'];    //订单号
        $pay_amount = sprintf("%.2f", $order['amount']);    //交易金额
        $pay_applydate = date("Y-m-d H:i:s");  //订单时间
        $pay_notifyurl =  $this->config['notify_url'];   //服务端返回地址
        $pay_callbackurl = "http://wspay.yayamy.com/Pay_Index.html";  //页面跳转返回地址
        $Md5key = "tfs7md2svzcc74kyn5niaxv2clh53sxu";
        ...
        $native['request_post_url'] ="http://wspay.yayamy.com/Pay_Index.html";
    ...
    {
        //获取预下单
        $url = self::getPayUnifiedOrder($params,'996');
        return [
            'request_url' => $url,
        ];
    }
    ...
    {
        Log::error('Post data from wangshang' . json_encode($_REQUEST));
